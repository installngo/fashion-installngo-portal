...
export default function CourseForm({ initialData, onDone }: CourseFormProps) {
  ...
  const [subCategories, setSubCategories] = useState<SubCode[]>([]);
  ...
  // Fetch master data
  useEffect(() => {
    async function fetchMasterData(codeType: string): Promise<MasterOption[]> {
    ...
    fetchMasterData("FASHIONCATEGORY").then((cats) => {
      ...
      if (!initialData?.category_code) {
        const defaultCat = cats.find((c) => c.is_default);
    ...
    fetchMasterData("COURSEVALIDITY").then((vals) => {
      ...
      if (!initialData?.validity_code) {
        const defaultVal = vals.find((v) => v.is_default);
